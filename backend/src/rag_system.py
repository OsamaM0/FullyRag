...
def _prefix_columns_in_where_clause(clause_str: str, prefix: str = "js.") -> str:
    ...
    for part_idx, part_content in enumerate(parts):
        ...
        match = re.match(
            r"^\s*([a-zA-Z_][a-zA-Z0-9_]*)\s+(.+)$", part_content.strip(), re.IGNORECASE
        )
        ...
        if match:
            ...
            if (
                column_candidate.upper() not in sql_keywords
                and "." not in column_candidate
                and "(" not in column_candidate
                and not column_candidate.replace(".", "", 1).isdigit()
                and column_candidate.upper() not in {"TRUE", "FALSE", "NULL"}
            ):
...
class SherpaDocumentProcessor:
    ...
    def _process_sherpa_data(self, sherpa_data):
        ...
        for block_data in sherpa_data:
            ...
            block = DocumentBlock(block_idx=metadata.block_idx, content=content, metadata=metadata)
...
class RAGSystem:
    ...
    def index_document(
        ...
        existing_sherpa_data=None,
        table_name=f"{schema_app_data}.rag_document_blocks",
    ):
    ...
    def query(
        ...
        for element in processed_query:
            sanitized = (
                element.replace("|", "").replace("!", "").replace("(", "").replace(")", "").strip()
            )
            if sanitized:
                formatted_elements.append(
                    f"({' & '.join(sanitized.split())})" if " " in sanitized else sanitized
                )
        ...
        if source_query:
            ...
            if len(_from_parts) < 2:
                ...
                _table_name_part = re.split(
                    r"\sWHERE\s|\sORDER BY\s|\sLIMIT\s|\sOFFSET\s",
                    _after_from,
                    1,
                    re.IGNORECASE,
                )[0].strip()
                ...
                _orderby_match = re.search(
                    r"\sORDER BY\s(.*?)(?:\sLIMIT\s|\sOFFSET\s|$)",
                    _after_from,
                    re.IGNORECASE | re.DOTALL,
                )
                ...
                if table_name:
                    ...
                    if raw_where_from_source:
                        prefixed_where = _prefix_columns_in_where_clause(
                            raw_where_from_source, "js."
                        )
                        if prefixed_where:
                            where_join_conditions_list.append(f"({prefixed_where})")

                    if raw_orderby_from_source:
                        order_by_join_clause_str = _prefix_columns_in_order_by_clause(
                            raw_orderby_from_source, "js."
                        )
        ...
        if formatted_elements:
            ...
            where_conditions.append(
                f"r.content_tsv @@ to_tsquery('{self.LANGUAGE}', '{{ts_query}}')"
            )
        ...
        if formatted_elements and (
            not order_by_join_clause_str or "score" not in order_by_join_clause_str.lower()
        ):
        ...
        if final_order_by_clauses:
            ...
            valid_clauses = [
                clause for clause in final_order_by_clauses if clause and clause.strip()
            ]
            if valid_clauses:
        ...
        doc_count_query_parts = [
            (part.replace("COUNT(*)", "COUNT(DISTINCT r.name)") if "COUNT(*)" in part else part)
            for part in count_query_parts
        ]
        ...
        total_document_count = (
            doc_count_result[0][0] if doc_count_result and doc_count_result[0] else 0
        )
        ...
        if total_count_to_return == 0 and formatted_elements:
            ...
            total_count_to_return = (
                count_result_or[0][0] if count_result_or and count_result_or[0] else 0
            )

            # Also update document count for OR search
            doc_count_query_parts_or = [
                (part.replace("COUNT(*)", "COUNT(DISTINCT r.name)") if "COUNT(*)" in part else part)
                for part in count_query_parts
            ]
            _final_doc_count_query_or = (
                "\n".join(doc_count_query_parts_or)
                .format(ts_query=ts_query_or_for_format)
                .replace(";", "")
            )
            doc_count_result_or = self.db_manager.execute_query(_final_doc_count_query_or)
            total_document_count = (
                doc_count_result_or[0][0] if doc_count_result_or and doc_count_result_or[0] else 0
            )
    ...
    def debug_blocks(self, pdf_file):
        ...
        table_name = (
            f"{schema_app_data}.rag_document_blocks"
            if rag_count > 0
            else f"{schema_app_data}.uploaded_document_blocks"
        )
        ...
        if rag_count == 0 and upload_count == 0:
            ...
            print(
                f"Found {rag_count} blocks in rag_document_blocks and {upload_count} blocks in uploaded_document_blocks. Using {table_name}."
            )
