...
def get_postgres_saver() -> BaseCheckpointSaver:
    ...
    if not db_manager:
        logger.error(
            "DatabaseManager not available for get_postgres_saver. Check logs for initialization errors."
        )
    ...
    try:
        ...
        logger.info(
            f"Attempting to initialize AsyncPostgresSaver with connection string: {conn_string}"
        )
        ...
        logger.error(
            f"Connection string used: {db_manager.get_connection_string() if db_manager else 'N/A'}"
        )
