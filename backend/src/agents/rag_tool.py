...
def query_rag(
    ...
    if source_names is not None and source_query is not None:
        return {
            "error": "Provide either 'source_names' (list of strings) or 'source_query' (SQL string), but not both."
        }
...
def query_rag_from_id_func(
    ...
    if isinstance(block_indices, int | str):
        try:
            ...
            try:
                ...
                return [
                    {
                        "error": "Invalid block_indices format. Must be an int, list of ints, or JSON string of ints."
                    }
                ]
    ...
    if not blocks:  # get_blocks_by_idx should return a list
        return [
            {"error": "No blocks found with the provided indices"}
        ]  # Return list with error dict
    ...
    try:
        ...
        block_indices_list = [
            block["block_idx"]
            for block in blocks
            if isinstance(block, dict) and "block_idx" in block
        ]
    ...
    for block in blocks:
        if not isinstance(block, dict):
            continue  # Skip if block is not a dict
...
def highlight_pdf_func(pdf_requests: list[dict[str, Any]], debug: bool | None = False) -> str:
    """
    ...
    for request in pdf_requests:
        ...
        if not pdf_file or not isinstance(pdf_file, str):
            results.append(
                {
                    "error": "Missing or invalid 'pdf_file' in request.",
        ...
        if exact_match:
            ...
            similar_matches = rag_system.db_manager.execute_query(
                like_query, (f"%{cleaned_pdf_file}%",)
            )

            if not similar_matches:
                results.append(
                    {
                        "error": f"PDF '{pdf_file}' not found in the RAG system (no exact or similar matches).",
                        "original_request": request,
                    }
                )
                continue
                ...
                print(
                    f"Exact match for '{pdf_file}' not found. Using similar match: '{found_pdf_name}'"
                )
            else:
                possible_names = [match[0] for match in similar_matches]
                results.append(
                    {
                        "error": f"PDF '{pdf_file}' not found. Multiple similar documents exist: {', '.join(possible_names)}",
                        "original_request": request,
                    }
                )
                continue
