...
def _execute_safe_query(query: str, params: tuple = None) -> list[tuple]:
    ...
    try:
        ...
        logger.error(
            f"Error executing query via DatabaseManager: {error_message} SQL: {query} PARAMS: {params}"
        )
...
def _get_letter_names_from_subquery(subquery: str) -> list[str]:
    ...
    try:
        ...
        if not letter_names:
            raise ValueError(
                "Subquery did not return any valid letter names from public.public_data."
            )
        return letter_names
    except Exception as e:
...
def _get_demands_data(
    ...
    priority: int | None = None,  # Made priority optional
    letter_names: list[str] | None = None,
    letter_name_subquery: str | None = None,
    search_text: str | None = None,
    limit: int | None = None,
    offset: int | None = None,
) -> tuple[list[dict], int] | int:  # For content: (list_of_dicts, total_count), for count: int
    """
    ...
    if return_type == "content":
        select_clause = (
            "d.demand_text, l.name, d.priority"  # Select demand text, letter name, and priority
        )
    ...
    if search_text:
        ...
        conditions.append("d.demand_tsv @@ plainto_tsquery(%s, %s)")
    ...
    if return_type == "count":
        ...
        if search_text:
            count_query_parts.append("d.demand_tsv @@ plainto_tsquery(%s, %s)")
...
def get_demand_content(
    demand_type: (
        Literal["Demandes à traiter prioritairement", "Autres demandes"] | None
    ) = None,  # Made optional
    letter_names: list[str] | None = None,
    letter_name_subquery: str | None = None,
    search_text: str | None = None,
    limit: int | None = 100,  # Default limit
    offset: int | None = 0,  # Default offset
...
def count_demands(
    ...
    try:
        ...
        if demand_type:
            ...
            if (
                priority is None and demand_type is not None
            ):  # Check if demand_type was given but not mapped
...
def execute_sql_func(sql_query: str) -> str:
    ...
    try:
        ...
        with (
            conn.cursor() as current_cursor
        ):  # Use 'current_cursor' to avoid conflict if 'cursor' was a global
            ...
            if not current_cursor.description:  # Fallback if no description
                if results:
                    ...
                    num_cols = (
                        len(results[0]) if results and isinstance(results[0], list | tuple) else 1
                    )
                    if num_cols == 1:
