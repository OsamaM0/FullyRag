...
def handle_tool_result(state: GraphingAgentState) -> GraphingAgentState:
    ...
    if (
        ...
        or tool_output.startswith("SQL Execution Error:")
        or tool_output.startswith("Data Error:")
        or tool_output.startswith("Input Data Error:")
        or tool_output.startswith("ValueError:")
    ):
        ...
        if is_sql_error or retries_exhausted:
            ...
            print(
                f"Graphing Agent: {'SQL Error' if is_sql_error else 'Max retries reached'}. Stopping. Error: {tool_output}"
            )
            return {"error": tool_output}  # Store final error and signal end
        else:
            # Increment retry count for non-SQL errors
            print(
                f"Graphing Agent: Tool failed (Attempt {current_retry_count + 1}). Retrying. Error: {tool_output}"
            )
            return {
                "retry_count": current_retry_count + 1,
                "error": tool_output,
            }  # Update retry count, keep error for next model call
    else:
        # Success - tool returned a graph_id
        print(f"Graphing Agent: Tool succeeded. Graph ID: {tool_output}")
        return {
            "graph_id": tool_output,
            "error": None,
            "retry_count": 0,
        }  # Store graph_id, clear error, reset retries
...
graph_builder.add_conditional_edges(
    "handle_tool_result", route_from_tool_result, {"model": "model", END: END}
)
...
if __name__ == "__main__":
    ...
    async def main_test():
        ...
        print(
            f"--- Running graphing agent with instructions (English, with data): {instructions} ---"
        )
        result_state_en_data = await run_graphing_agent(
            input_data=sample_data, graph_instructions=instructions, language="english"
        )
        ...
        result_state_fr_data = await run_graphing_agent(
            input_data=sample_data,
            graph_instructions=instructions_french,
            language="french",
        )
        ...
        result_state_sql = await run_graphing_agent(
            query_string=test_query_string,
            graph_instructions=instructions_sql,
            language="english",
        )
        ...
        result_state_llm_query = await run_graphing_agent(
            input_data=None,
            query_string=None,
            graph_instructions=instructions_sql,
            language="english",
