...
def create_graph(
    query: str | None = None,  # SQL query to fetch data (use either query or data)
    data: (
        list[dict[str, Any]] | None
    ) = None,  # Direct data input as list of dicts (use either query or data)
    ...
    preprocess: dict[str, bool | str | list[str]] | None = None,  # preprocessing instructions
):
    """
    ...
    if data:
        ...
        try:
            ...
            raise ValueError(
                f"SQL Execution Error: Failed to execute query '{cleaned_query}'. Reason: {e}"
            )
    else:
    ...
    if "df" in locals() and df.empty:
        ...
        if query:  # check if the error originated from SQL or just empty direct data
            raise ValueError(
                "SQL Execution Error: The query executed successfully but returned no results."
            )
        else:
            ...
            if preprocess["create_date"] == "from_year_month":
                if "year" in df.columns and "month" in df.columns:
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Failed to create date column from year and month: {str(e)}"
                        )
                else:
                    raise ValueError(
                        "Cannot create date column: Either 'year' or 'month' column is missing"
                    )
            ...
            if isinstance(agg_config, dict):
                ...
                if group_by and agg_col:
                    if group_by in df.columns and agg_col in df.columns:
                        ...
                        raise ValueError(
                            f"Cannot aggregate data: Missing columns: {', '.join(missing_cols)}"
                        )
    ...
    if "df" in locals() and not df.empty:
        if x_col and x_col not in df.columns:
            ...
            raise ValueError(
                f"Data Error: Column '{x_col}' for X-axis not found. Available columns: {columns_str}"
            )
        if y_col and y_col not in df.columns:
            columns_str = ", ".join(df.columns)
            raise ValueError(
                f"Data Error: Column '{y_col}' for Y-axis not found. Available columns: {columns_str}"
            )
        if color_col and color_col not in df.columns:
            columns_str = ", ".join(df.columns)
            raise ValueError(
                f"Data Error: Column '{color_col}' for color not found. Available columns: {columns_str}"
            )
        if size_col and size_col not in df.columns:
            columns_str = ", ".join(df.columns)
            raise ValueError(
                f"Data Error: Column '{size_col}' for size not found. Available columns: {columns_str}"
            )
        ...
        raise ValueError(
            "Data Error: Cannot proceed with plotting as data is unavailable or empty."
        )
    ...
    if chart_type == "bar":
        ...
        if not x_col:
            raise ValueError(
                "For a pie chart, please specify 'x_col' for category names (or 'values' for quantities)."
            )
        fig = px.pie(
            df,
            names=x_col,
