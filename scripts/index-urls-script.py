    ...
    try:
        ...
        if "temp_pdf_path" in locals() and os.path.exists(temp_pdf_path):
            try:
                ...
                print(
                    f"Warning: Could not remove temporary file {temp_pdf_path} after error: {e_clean}"
                )
        return None
...
def main():
    ...
    parser.add_argument(
        "--table",
        required=False,
        help="Database table name containing URLs",
        default="arxiv_qbio",
    )
    parser.add_argument(
        "--column",
        required=False,
        help="Column name containing PDF URLs",
        default="pdf_url",
    )
    parser.add_argument(
        "--embeddings",
    ...
    try:
        ...
        if not (
            args.schema.isidentifier() and args.table.isidentifier() and args.column.isidentifier()
        ):
        ...
        pdf_urls = [
            row[0] for row in results if row[0] and isinstance(row[0], str) and is_valid_url(row[0])
        ]
