...
try:
    ...
    summarization_prompt_template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are an expert data analyst. Given the following information about a database column and sample values, provide a concise, one-sentence summary describing the nature of the data in this column. Focus on data type, range, common patterns, or purpose if discernible. Be brief and informative.
...
def generate_db_rag_prompt(
    ...
    tables_to_include: dict[str, list[str]] | None = None,
    ...
    for schema in schemas_to_include:
        try:
            ...
            if tables_to_include and schema in tables_to_include:
                target_tables = [
                    tbl for tbl in all_tables_in_schema if tbl in tables_to_include[schema]
                ]
                ...
                try:
                    ...
                    for i, col in enumerate(columns):
                        ...
                        if col_name in embedding_cols:
                            table_descriptions.append(
                                "   - **Note**: This column contains embeddings, suitable for semantic similarity searches (e.g., using vector operators like `<=>`)."
                            )
                        if col_name in tsvector_cols:
                            table_descriptions.append(
                                "   - **Note**: This column contains tsvector data, suitable for full-text keyword searches (e.g., using functions like `to_tsquery` and `@@`)."
                            )
                        ...
                        if include_summary or examples_to_show > 0:
                            try:
                                ...
                                logger.error(
                                    f"Error sampling column {schema}.{table}.{col_name}: {sample_error}"
                                )
                                table_descriptions.append(
                                    f"   - **Summary**: (Error retrieving samples: {sample_error})"
                                )
                    ...
                    table_descriptions.append(
                        f"  - (Error retrieving details for this table: {table_error})"
                    )
