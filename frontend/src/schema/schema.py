...
class UserInDB(UserBase):  # Represents a user object as stored in or retrieved from the DB
    ...
    updated_at: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp of last user update."
    )
...
class Feedback(BaseModel):  # type: ignore[no-redef]
    ...
    commented_message_text: str | None = Field(
        default=None,
...
class AnnotationsRequest(BaseModel):
    ...
    user_id: str | UUID | None = Field(default=None, description="Optional User ID for context.")
