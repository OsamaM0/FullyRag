...
# Initialize session state at module load
init_session_state()
...
st.session_state.confirming_delete_thread_id = st.session_state.get(
    "confirming_delete_thread_id", None
)
...
dialog_title = (
    dt.PDF_DIALOG_TITLE
    if not st.session_state.debug_viewer
    else dt.PDF_DIALOG_DEBUG_PREFIX + " " + str(st.session_state.pdf_to_view)
)
...
def pdf_dialog():
    ...
    if pdf_name and current_agent_client:
        try:
            display_pdf(
                agent_client=current_agent_client,
        ...
        if not current_agent_client:
            st.error(dt.AGENT_CLIENT_NOT_AVAILABLE)
...
async def main() -> None:
    ...
    # Show warning if user manually refreshed the page
    # This happens when session is fresh but we lost authentication
    if "session_initialized" not in st.session_state:
        st.session_state.session_initialized = True
        # This is a fresh session - authentication check will happen in ensure_authenticated()
    ...
    # Initialize AgentClient if not present or still None
    if st.session_state.get("agent_client") is None:
        load_dotenv()
        ...
        try:
            with st.spinner(dt.AGENT_CONNECTING_SPINNER):
                # Pass the access token from session state if available
                access_token = st.session_state.get("access_token")
                st.session_state.agent_client = AgentClient(
                    base_url=agent_url, access_token=access_token
                )
    ...
    if query_thread_id and (
        "thread_id" not in st.session_state or query_thread_id != st.session_state.thread_id
    ):
        try:
            messages_history = agent_client.get_history(
                thread_id=query_thread_id
            )  # Pass user_id if backend supports
            ...
            try:
                ...
                title = agent_client.get_conversation_title(
                    query_thread_id, user_id=current_user_id
                )
                st.session_state.conversation_title = title
            except:
                st.session_state.conversation_title = dt.DEFAULT_CONVERSATION_TITLE
        except AgentClientError:
            st.error(dt.CONVERSATION_LOAD_ERROR)
    ...
    with st.sidebar:
        # Language selector
        from display_texts import get_available_languages, get_current_language, set_language
        ...
        # Ensure service info is available before accessing models
        if getattr(agent_client, "info", None) is None:
            try:
                agent_client.retrieve_info()
            except AgentClientError as e:
                st.error(dt.AGENT_CONNECTION_ERROR.format(agent_url=agent_client.base_url, e=e))
                st.info(
                    "Tip: Ensure frontend AUTH_SECRET matches backend AUTH_SECRET in your .env files."
                )
                st.stop()

        try:
            agent_client.info.models.index(agent_client.info.default_model)  # type: ignore[union-attr]
            model = str(agent_client.info.default_model)  # type: ignore[union-attr]
        except Exception:
            # Fallback if models/default_model are unavailable or mismatched
            model = "gpt-4o"
        ...
        if st.button(
            ...
            # Persist a placeholder conversation immediately so it appears in recents
            try:
        ...
        if "conversation_title" not in st.session_state:
            try:
                title = agent_client.get_conversation_title(
                    st.session_state.thread_id, user_id=current_user_id
                )
                st.session_state.conversation_title = title
            except:
                st.session_state.conversation_title = dt.DEFAULT_CONVERSATION_TITLE
        ...
        if st.session_state.get("editing_title", False):
            ...
            with col1:
                if st.button(dt.SAVE_TITLE_BUTTON, key="save_title"):
                    ...
                    agent_client.set_conversation_title(
                        st.session_state.thread_id, new_title, user_id=current_user_id
                    )
            ...
            if conversations:
                ...
                for conv in conversations:
                    ...
                    col1, col2, col3 = st.columns(
                        [0.9, 0.05, 0.05]
                    )  # Adjusted for potentially wider titles
                    with col1:
                        if st.button(
                            f"{title}",
                            key=f"conv_{thread_id_conv}",
                            help=f"{dt.CONVERSATION_LAST_UPDATED_HELP.format(date_str=date_str)}",
                            type="tertiary",
                        ):
                    ...
                    with col2:
                        if st.button(
                            ...
                            help=dt.EDIT_CONVERSATION_TITLE_HELP,
                            type="tertiary",
                        ):
                            st.query_params["thread_id"] = thread_id_conv
                ...
                for i in range(0, num_prompts, 2):
                    ...
                    with cols[0]:
                        ...
                        if st.button(
                            ...
                            st.session_state.suggested_command = prompt_data[
                                "suggested_command_text"
                            ]
                            hide_welcome()
                            st.rerun()
                    ...
                    if i + 1 < num_prompts:
                        with cols[1]:
                            ...
                            if st.button(
                                ...
                                st.session_state.suggested_command = prompt_data_next[
                                    "suggested_command_text"
                                ]
                                hide_welcome()
                                st.rerun()
    ...
    if (
        user_input := st.chat_input(
            dt.CHAT_INPUT_PLACEHOLDER, accept_file="multiple", file_type=["pdf"]
        )
        or st.session_state.suggested_command
    ):
        ...
        if st.session_state.suggested_command:
            ...
            user_input
        ):  # Check if user_input is not None (it can be if only suggested_command was true)
            user_text = user_input.text
            files = (
                user_input.files if hasattr(user_input, "files") else []
            )  # Ensure files attribute exists

        if (
            user_text is None and not files
        ):  # Handle case where only suggested_command was true but it was empty
            ...
            messages.append(
                ChatMessage(
                    ...
                    attached_files=[f.name for f in files],
                )
            )
            additional_markdown = ""
            if files:
                ...
                for file_name_obj in files:
                    additional_markdown += dt.FILE_ATTACHED_BADGE.format(
                        file_name=file_name_obj.name
                    )
            st.chat_message("human", avatar=USER_ICON).write(
                (user_text or "") + additional_markdown
            )
                ...
                for file_obj in files:
                    ...
                    try:
                        ...
                        upload_status.update(
                            label=dt.FILE_UPLOAD_SUCCESS_STATUS.format(file_name=file_name)
                        )
                    except Exception as e:
                        upload_status.error(
                            dt.FILE_UPLOAD_ERROR_STATUS.format(file_name=file_name, e=e)
                        )
                upload_status.update(
                    state="complete", label=f"{len(uploaded_file_ids)} files uploaded"
                )
            try:
                ...
                if (
                    len(messages) > 1
                    and st.session_state.conversation_title == dt.DEFAULT_CONVERSATION_TITLE
                ):
                    try:
                        title_prompt = dt.TITLE_GENERATION_PROMPT.format(
                            user_text=user_text or "Uploaded files"
                        )
                        # TODO: Ensure ainvoke for title is user-scoped if necessary
                        title_response = await agent_client.ainvoke(
                            message=title_prompt, model=model
                        )  # user_id?
                        ...
                        # Ensure the conversation is at least persisted with default title
                        try:
                            await agent_client.aset_conversation_title(
                                st.session_state.thread_id,
                                st.session_state.conversation_title
                                or dt.DEFAULT_CONVERSATION_TITLE,
                                user_id=current_user_id,
                            )
                        except Exception:
                            pass
...
async def draw_messages(
    ...
    current_user_id = st.session_state.get(
        "current_user_id"
    )  # Get current user for any user-specific logic

    while msg := await anext(messages_agen, None):
        if isinstance(msg, tuple):
            msg, additional_data = msg
            if (
                not st.session_state.get("additional_data_added", False)
                and additional_data is not None
                and len(st.session_state.get("messages", [])) > 0
            ):
                additional_markdown = ""
                for definition in additional_data:
                    additional_markdown += "  \n" + dt.LEXICON_BADGE.format(
        ...
        match msg.type:
            case "human":
                ...
                if hasattr(msg, "attached_files") and msg.attached_files:
                    additional_markdown = "  \n"
                    for file_name in msg.attached_files:
                ...
                with st.session_state.last_message:
                    if msg.content:
                        if streaming_placeholder:  # End of stream for this message part
                            ...
                            st.write(msg.content)  # Non-streamed AI message
                    ...
                    if msg.tool_calls:
                        ...
                        for tool_call in msg.tool_calls:
                            ...
                            if tool_call["name"] == "Graphing_Agent":
                                Create_Graph_call_ids.add(tool_call["id"])
...
async def chain_messages(
    ...
    next_messages: AsyncGenerator[ChatMessage | str, None],
...
async def process_tool_result(
    ...
    if not status:
        st.error(dt.TOOL_CALL_ID_NOT_FOUND.format(tool_call_id=tool_result.tool_call_id))
        return
    ...
    if tool_result.tool_call_id in Create_Graph_call_ids and agent_client:
        ...
        with status:
            try:
                ...
                status.update(
                    state="running",
                    label=dt.PDF_PROCESSING_STATUS.format(count=len(pdf_results_list)),
                )

                all_processed_successfully = True
                # pdf_buttons_to_create is already initialized outside

                for pdf_entry_idx, pdf_entry in enumerate(
                    pdf_results_list
                ):  # Added enumerate for unique key generation
                    if pdf_entry.get("error"):
                        original_pdf_file_name = pdf_entry.get("original_request", {}).get(
                            "pdf_file", f"request_{pdf_entry_idx}"
                        )
                        st.error(
                            dt.PDF_ERROR_FOR_FILE.format(
                                original_pdf_file_name=original_pdf_file_name,
                                error=pdf_entry["error"],
                            )
                        )
                    ...
                    if agent_client:
                        try:
                            if not debug_viewer:
                                annotations = await agent_client.aget_annotations(
                                    pdf_file=pdf_name,
                                    block_indices=block_indices,
                                    user_id=user_id,
                                )
                            else:
                                annotations = await agent_client.adebug_pdf_blocks(
                                    pdf_file=pdf_name, user_id=user_id
                                )
                ...
                if all_processed_successfully and pdf_buttons_to_create:
                    status.update(
                        state="complete",
                        label=dt.PDF_ALL_PROCESSED.format(count=len(pdf_buttons_to_create)),
                    )
        ...
        with st.container(key=f"sources_{tool_result.tool_call_id}", border=True):
            st.button(
                ...
                type="tertiary",
                icon=":material/info:",
                help=dt.HELP_SOURCE_PDFs,
                key=f"pdf_{str(uuid.uuid4())}",
            )
            with st.container(key=f"pdf_buttons_container_{tool_result.tool_call_id}"):
                for btn_data in pdf_buttons_to_create:
                    button_key = (
                        f"pdf_button_{btn_data['tool_call_id']}_{btn_data['unique_suffix']}"
                    )
                    if st.button(
                        btn_data["name"],
                        key=button_key,
                        ...
                        view_pdf(
                            btn_data["name"],
                            btn_data["annotations"],
                            debug_viewer=btn_data["debug"],
                        )
    else:
        with status:
            ...
            if current_tool_name == "SQL_Executor":
                ...
                if data_lines:
                    try:
                        ...
                        st.error(dt.SQL_PARSING_ERROR.format(e=e))
                        st.code(csv_string, language="csv")
                else:
                    st.info(dt.SQL_NO_DATA)
...
async def handle_feedback() -> None:
    ...
    if (
        feedback_stars is not None
        and (latest_run_id, feedback_stars) != st.session_state.last_star_feedback
    ):
        normalized_score = (feedback_stars + 1) / 5.0
        agent_client: AgentClient = st.session_state.agent_client
        conversation_id = st.session_state.thread_id
        commented_message_text = (
            st.session_state.messages[-1].content if st.session_state.messages else None
        )
        try:
