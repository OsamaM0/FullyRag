...
class AgentClient:
    ...
    def _headers(self) -> dict[str, str]:
        headers = {}
        # Priority: service AUTH_SECRET > JWT token (backend protected routes require AUTH_SECRET)
        if self.auth_secret:
    ...
    async def ainvoke(
        ...
        user_id: str | UUID | None = None,  # Added user_id
    ) -> ChatMessage:
        """
        ...
        if line.startswith("data: "):
            ...
            match parsed["type"]:
                case "message":
                    ...
                    try:
                        return (
                            ChatMessage.model_validate(parsed["content"]),
                            additional_data,
                        )
                    except Exception as e:
    ...
    def create_feedback(self, user_id: UUID, feedback: str) -> None:
        """
    ...
    def get_conversations(
        self, limit: int = 20, user_id: str | UUID | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def set_conversation_title(
        self, thread_id: str, title: str, user_id: str | UUID | None = None
    ) -> None:
        """
    ...
    def get_conversation_title(self, thread_id: str, user_id: str | UUID | None = None) -> str:
        """
    ...
    def delete_conversation(self, thread_id: str, user_id: str | UUID | None = None) -> bool:
        """
    ...
    async def aget_conversation_title(
        self, thread_id: str, user_id: str | UUID | None = None
    ) -> str:
        """
    ...
    async def aget_annotations(
        self, pdf_file: str, block_indices: list[int], user_id: str | UUID | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    async def adebug_pdf_blocks(
        self, pdf_file: str, user_id: str | UUID | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def get_document_source_status(self, document_name: str) -> dict[str, Any] | None:
        """
    ...
    async def aget_document_source_status(self, document_name: str) -> dict[str, Any] | None:
        """
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise AgentClientError(
                    f"HTTP Status Error getting document source status: {e.response.status_code} - {e}"
                )
            except httpx.HTTPError as e:
    ...
    def submit_user_feedback(self, user_id: UUID, feedback_content: str) -> UserFeedbackRead:
        ...
        try:
            ...
            raise AgentClientError(
                f"An unexpected error occurred while submitting user feedback: {e}"
            )
            ...
            try:
                response = await client.post(
                    ...
                    json=request_data.model_dump(
                        mode="json"
                    ),  # Ensure UUID is serialized correctly
                    headers=self._headers,
                    timeout=self.timeout,
                )
                response.raise_for_status()
    ...
    def login(self, email: str, password: str) -> dict:
        ...
        Returns:
            Dictionary with access_token, refresh_token, user_id, and email
        """
        try:
            response = httpx.post(
            ...
            try:
                ...
                error_detail = str(e)
            raise AgentClientError(f"Login failed: {error_detail}")

    async def alogin(self, email: str, password: str) -> dict:
        """
        Async login with email and password to get JWT tokens.
    ...
    def register(self, email: str, password: str) -> dict:
        ...
        Returns:
            Dictionary with user_id, email, and message
        """
        try:
            response = httpx.post(
            ...
            try:
                ...
                error_detail = str(e)
            raise AgentClientError(f"Registration failed: {error_detail}")

    async def aregister(self, email: str, password: str) -> dict:
        """
        Async register a new user account.
    ...
    def refresh_access_token(self) -> str:
        ...
        if not self.refresh_token:
            raise AgentClientError("No refresh token available")
        ...
        try:
            ...
            try:
                ...
                error_detail = str(e)
            raise AgentClientError(f"Token refresh failed: {error_detail}")
    ...
    def logout(self) -> None:
        ...
        try:
            ...
            pass  # Logout on backend is optional
        finally:
            self.clear_tokens()
    ...
    def get_current_user(self) -> dict:
        ...
        try:
            ...
            try:
                ...
                error_detail = str(e)
            raise AgentClientError(f"Failed to get user info: {error_detail}")
